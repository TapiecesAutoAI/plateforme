...
export async function POST(request: NextRequest) {
  ...
  const organizationId = form.get("organizationId")?.toString().trim();
  ...
  if (!["image/jpeg", "image/jpg", "image/pjpeg", "image/png", "image/webp"].includes(file.type)) {
  ...
  const targetOrganizationId =
    session.accessRole === "super_admin" ? organizationId : session.organizationId;
  ...
  const blob = await put(
    `tpa/organizations/${targetOrganizationId}/sellers/${customerId}${extension}`,
    file,
    {
      access: "private",
