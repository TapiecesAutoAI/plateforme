...
for (const profile of profiles) {
  ...
  while (
    ...
    if (
      ...
      if (id === "problem-aged-battery") {
        const probs =
          result.reasoning.decision.probabilities;
    ...
    if (
      ...
      if (
        ...
        if (id === "problem-aged-battery") {
          const probs =
            completed.reasoning.decision.probabilities;
  ...
  output.push(
    `Nombre aged : ${forcedAgedPaths.length}`,
  );
  ...
  for (
    const line
    of forcedAgedPaths.slice(0, 100)
  ) {
    output.push(line);
  }
