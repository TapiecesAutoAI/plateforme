...
for (const profile of profiles) {
  ...
  output.push(
    "Distribution hypothese dominante :",
  );
  ...
  output.push(
    "20 PREMIERS MANUAL REVIEWS :",
  );

  for (
    const item
    of manualReviewPaths.slice(
      0,
      20,
    )
  ) {
    output.push(
      `TOP=${item.topHypothesis} | CONF=${(item.topProbability * 100).toFixed(2)}% | QUESTIONS=${item.completedQuestions}`,
    );
