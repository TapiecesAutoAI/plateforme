...
for (const profile of profiles) {
  ...
  let anomalies = 0;
  let manualReviews = 0;
    const exploredByDepth =
      new Map<number, number>();

    const ageOverFourByDepth =
      new Map<number, number>();

    const postLowByDepth =
      new Map<number, number>();

    const goodByDepth =
      new Map<number, number>();

    let firstAgeOverFourAt = -1;
    let firstPostLowAt = -1;
    let firstGoodAt = -1;
    let firstTripleAt = -1;
  ...
  while (
    ...
    }
    {
      ...
      if (age) {
        ageOverFourByDepth.set(
          depth,
          (
            ageOverFourByDepth.get(
              depth,
            ) ?? 0
          ) + 1,
        );
  ...
  output.push(
    "=== EXPLORATION BUDGET AUDIT ===",
  );

  output.push(
    `FIRST AGE >4       : ${firstAgeOverFourAt}`,
  );

  output.push(
    `FIRST POST <12.2    : ${firstPostLowAt}`,
  );

  output.push(
    `FIRST TEST GOOD     : ${firstGoodAt}`,
  );

  output.push(
    `FIRST TRIPLE        : ${firstTripleAt}`,
  );
  ...
  for (
    ...
    output.push(
      [
        `DEPTH=${depth}`,
        `ALL=${exploredByDepth.get(depth) ?? 0}`,
        `AGE=${ageOverFourByDepth.get(depth) ?? 0}`,
        `POST=${postLowByDepth.get(depth) ?? 0}`,
        `GOOD=${goodByDepth.get(depth) ?? 0}`,
      ].join(" | "),
    );
  }
